...
const seedLocalDatabase = async () => {
  ...
  try {
    ...
    console.log('\n🎉 In-memory database seeded successfully!');
    console.log('═══════════════════════════════════');
    ...
    console.log('💡 Server will keep this in-memory database running');
...
module.exports = { seedLocalDatabase };

// Run the seed script if called directly
if (require.main === module) {
  seedLocalDatabase().then(() => {
    console.log('✅ Seeding completed. Keeping server alive...');
    // Keep the process alive to maintain the in-memory database
    setInterval(() => {
      console.log('⏰ In-memory database is still running...');
    }, 30000);
  });
}
